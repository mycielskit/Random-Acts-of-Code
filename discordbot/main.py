...
Author = 'INSERT CODE HERE'
Friend = 'INSERT CODE HERE'
...
async def on_message(msg):
    if msg.author.bot or msg.content[0] != '$':
        if msg.author.bot:
            server = client.get_guild(INSERT GUILD HERE)
            if len(server.members) == 1004:
                botcommands = client.get_channel(INSERT CHANNEL HERE)
    ...
    cmd = msg.content[1:]
    if cmd == "exit":
        botcommands = client.get_channel(624056396639502336)
